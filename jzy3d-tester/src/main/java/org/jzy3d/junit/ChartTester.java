 ...
 * 
 * A chart having a screenshot different from the test case image will generate
 * throw a ChartTestFailed exception.
 * 
 * @author martin
 */
public class ChartTester {
	...
	/*
	 * In Java, a mouse click only registers if the mouse is pressed and released
	 * without moving the mouse at all. This is difficult for most users to
	 * accomplish, so most UI elements (like buttons) react to the mouse press and
	 * release events and ignore the "click".
	 */
	 ...
	 * <code>data/tests/error-[name].png</code>. This image is always deleted before
	 * running a testcase.
	 * 
	 ...
	 * @param chart
	 * @param testImage
	 * @throws IOException if a non chart related error occurs. Actual chart test
	 *                     errors call <code>fail(...)</code>
	 */
	...
	public void test(Chart chart, String testImage) throws IOException {
		try {
			...
			String expectFile = getTestCaseFailedFileName() + new File(testImage).getName().replace(".", "#EXPECT#.");
	 ...
	 */
	protected void pixelHighlight(BufferedImage expected, List<IntegerCoord2d> diffs, Highlight highlight) {
	 ...
	 * @param filename
	 * @throws IOException
	 * @throws ChartTestFailed is thrown if at least one pixel differ. The exception
	 *                         holds all pixel coordinates where a difference
	 *                         exists.
	 */
