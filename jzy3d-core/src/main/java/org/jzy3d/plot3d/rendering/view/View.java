 ...
 * {@link AbstractViewportManager}s such as the {@Camera} rendering the scene or
 * an {@link AWTImageViewport} for displaying an image in the same window. On
 * can control the {@link Camera} with a {@ViewController } and get notifyed by
 * a {@link IViewPointChangedListener} that the view point has changed. The
 * control is relative to the center of the {@link Scene} and is defined using
 * polar coordinates.
 * 
...
public class View {
	...
	/**
	 * display a magenta parallelepiped around the "whole bounds" of the axis (box +
	 * labels) for debugging purpose.
	 */
	...
	/**
	 * Applies a factor to the default camera distance which is set to the radius of
	 * the scene bounds. Changing this value also change the camera clipping planes.
	 */
	 ...
	 * view point is set at {@link View.DEFAULT_VIEW}.
	 * <p/>
	 * The {@link Quality} allows setting the rendering capabilities that are set
	 * one time by the init() method.
	 */
	 ...
	 * 
	 * Method name is made clear to avoid confusion with {@link View#init()} which
	...
	/**
	 * Might be invoked by a {@link IScreenCanvas} to indicate that dimension
	 * changed and that elements should be reprocessed at next rendering, e.g. 2d
	 * projections.
	 */
	...
	/**
	 * Set the surrounding {@link AxisBox} dimensions, the {@link Camera} target and
	 * the colorbar range.
	 */
	...
	/**
	 * Set the viewpoint using polar coordinates relative to the target (i.e. the
	 * center of the scene). Only X and Y dimensions are required, as the distance
	 * to center will be computed automatically by {@link updateCamera()}.
	 */
	 ...
	 * {@link updateCamera()}, which depends on the view scaling.
	 * 
	 * @see {@link setViewPoint(Coord3d polar, boolean updateView)}
	 */
	 ...
	 * {@link setSquared(boolean v)} status.
	 */
	...
	/**
	 * Select between an automatic bounding (that allows fitting the entire scene
	 * graph), or a custom bounding.
	 */
	 ...
	 * {ViewBoundMode.MANUAL}, meaning that any call to {@link updateBounds()} will
	 * update view bounds to the current bounds.
	 */
	...
	public void renderScene(float left, float right) {
		ViewportConfiguration vc = ViewportBuilder.column(canvas.getRendererWidth(), canvas.getRendererHeight(), left,
				right);
		renderScene(vc);
	}
	...
	/**
	 * Not called anymore as alpha and blending setting are not supposed to change
	 * during lifetime of a chart.
	 * 
	 * If this should change, then the entire {@link View#initQuality()} method
	 * should be invoked
	 */
	@Deprecated
	...
	/**
	 * Return a 3d scaling factor that allows scaling the scene into a square box,
	 * according to the current ViewBoundMode.
	 * <p/>
	...
	public Coord3d computeSceneScaling(Scene scene, boolean squared, ViewBoundMode boundmode, BoundingBox3d viewbounds,
			SpaceTransformer spaceTransformer) {
		if (squared)
			return squarify(scene, boundmode, viewbounds, spaceTransformer);
		else
	...
	public void updateCamera(ViewportConfiguration viewport, BoundingBox3d bounds, float sceneRadiusScaled) {
		updateCamera(viewport, bounds, sceneRadiusScaled, viewmode, viewpoint, cam, cameraMode, factorViewPointDistance,
				center, scaling);
	}
