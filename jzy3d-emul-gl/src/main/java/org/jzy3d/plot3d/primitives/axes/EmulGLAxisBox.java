...
public class EmulGLAxisBox extends AxisBox {
      ...
      if (!getQuadIsHidden()[quad]) {
        ...
        if ((quad != 0) && (quad != 1)) {
          ...
          for (int t = 0; t < xticks.length; t++) {
            painter.glBegin(GL.GL_LINES); // JOGL CONSTANT VALUE 2 -> THIS REQUIRES 1, recognized by
                                          // JGL
  ...
  /**
   * Override default behaviour to replace direct text rendering by tick labels collection for later
   * rendering.
   */
  @Override
