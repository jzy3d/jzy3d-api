...
// SQUARE MODE NOT SUPPORTED YET
		...
		/*
		cam.setViewportMode(ViewportMode.SQUARE);
		chart.getView().shoot();
		
		// Then viewport is SQUARE
		Assert.assertEquals(ViewportMode.SQUARE, cam.getLastViewPort().getMode());
		
		int sideLength = 100;
		Assert.assertEquals(sideLength, cam.getLastViewPort().getHeight());
