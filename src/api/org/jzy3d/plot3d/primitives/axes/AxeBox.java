...
public class AxeBox implements IAxe {
	...
	public void setAxe(BoundingBox3d bbox) {
		this.boxBounds = bbox;
		setAxeBox(bbox.getXmin(), bbox.getXmax(), bbox.getYmin(),
				bbox.getYmax(), bbox.getZmin(), bbox.getZmax());
	}
	...
	public void draw(GL gl, GLU glu, Camera camera) {
		...
		if (layout.isFaceDisplayed()) {
			...
			if (gl.isGL2()) {
				...
				gl.getGL2().glColor4f(quadcolor.r, quadcolor.g, quadcolor.b,
						quadcolor.a);
			} else {
				GLES2CompatUtils.glPolygonMode(GL2.GL_BACK, GL2.GL_FILL);
				GLES2CompatUtils.glColor4f(quadcolor.r, quadcolor.g,
						quadcolor.b, quadcolor.a);
			}
			gl.glLineWidth(1.0f);
		...
		if (gl.isGL2()) {
			...
			gl.getGL2().glColor4f(gridcolor.r, gridcolor.g, gridcolor.b,
					gridcolor.a);
			...
			GLES2CompatUtils.glColor4f(gridcolor.r, gridcolor.g,
		...
		if (xrange > 0 && layout.isXTickLabelDisplayed()) {
			...
			if (view != null && view.getViewMode().equals(ViewPositionMode.TOP)) {
				BoundingBox3d bbox = drawTicks(gl, glu, camera, 1, AXE_X,
						layout.getXTickColor(), Halign.LEFT, Valign.TOP); // setup
																			// tick
																			// labels
																			// for
																			// X
			...
			// otherwise computed placement
				...
				if (xselect >= 0) {
					BoundingBox3d bbox = drawTicks(gl, glu, camera, xselect,
							AXE_X, layout.getXTickColor());
					wholeBounds.add(bbox);
				} else {
					...
					BoundingBox3d bbox = drawTicks(gl, glu, camera, 2, AXE_X,
							layout.getXTickColor(), Halign.CENTER, Valign.TOP);
					wholeBounds.add(bbox);
				}
			}
		}
		...
		if (yrange > 0 && layout.isYTickLabelDisplayed()) {
			if (view != null && view.getViewMode().equals(ViewPositionMode.TOP)) {
				BoundingBox3d bbox = drawTicks(gl, glu, camera, 2, AXE_Y,
						layout.getYTickColor(), Halign.LEFT, Valign.GROUND); // setup
																				// tick
																				// labels
																				// for
																				// Y
				...
				if (yselect >= 0) {
					BoundingBox3d bbox = drawTicks(gl, glu, camera, yselect,
							AXE_Y, layout.getYTickColor());
					wholeBounds.add(bbox);
				} else {
					...
					// HACK: handles "on top" view, when all face of cube are
					// drawn, which forbid to select an axe automatically
					BoundingBox3d bbox = drawTicks(gl, glu, camera, 1, AXE_Y,
							layout.getYTickColor(), Halign.RIGHT, Valign.GROUND);
					wholeBounds.add(bbox);
				}
			}
		}
	 ...
	 */
	protected void setAxeBox(float xmin, float xmax, float ymin, float ymax,
			float zmin, float zmax) {
		// Compute center
		center = new Coord3d((xmax + xmin) / 2, (ymax + ymin) / 2,
				(zmax + zmin) / 2);
		xrange = xmax - xmin;
		yrange = ymax - ymin;
		zrange = zmax - zmin;
	...
	protected int findClosestXaxe(Camera cam) {
		...
		for (int a = 0; a < na; a++) {
			if (quadIsHidden[axeXquads[a][0]] ^ quadIsHidden[axeXquads[a][1]])
				distAxeX[a] = new Vector3d(axeXx[a][0], axeXy[a][0],
						axeXz[a][0], axeXx[a][1], axeXy[a][1], axeXz[a][1])
						.distance(cam.getEye());
			else
	...
	protected int findClosestYaxe(Camera cam) {
		...
		for (int a = 0; a < na; a++) {
			if (quadIsHidden[axeYquads[a][0]] ^ quadIsHidden[axeYquads[a][1]])
				distAxeY[a] = new Vector3d(axeYx[a][0], axeYy[a][0],
						axeYz[a][0], axeYx[a][1], axeYy[a][1], axeYz[a][1])
						.distance(cam.getEye());
			else
	...
	protected int findClosestZaxe(Camera cam) {
		...
		for (int a = 0; a < na; a++) {
			if (quadIsHidden[axeZquads[a][0]] ^ quadIsHidden[axeZquads[a][1]])
				distAxeZ[a] = new Vector3d(axeZx[a][0], axeZy[a][0],
						axeZz[a][0], axeZx[a][1], axeZy[a][1], axeZz[a][1])
						.distance(cam.getEye());
			else
		...
		for (int a = 0; a < na; a++) {
			if (distAxeZ[a] < Double.MAX_VALUE) {
				Coord3d axeCenter = new Coord3d(
						(axeZx[a][0] + axeZx[a][1]) / 2,
	...
	protected ITextRenderer txt = new TextBitmapRenderer(); // use this text
															// renderer to get
															// occupied volume
															// by text
