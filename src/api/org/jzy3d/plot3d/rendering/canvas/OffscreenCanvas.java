...
public class OffscreenCanvas implements ICanvas {
	...
	public OffscreenCanvas(IChartComponentFactory factory, Scene scene,
			Quality quality, GLCapabilities capabilities, int width, int height) {
		this(factory, scene, quality, capabilities, width, height, false, false);
	}

	public OffscreenCanvas(IChartComponentFactory factory, Scene scene,
			Quality quality, GLCapabilities capabilities, int width,
			int height, boolean traceGL, boolean debugGL) {
		view = scene.newView(this, quality);
		renderer = factory.newRenderer(view, traceGL, debugGL);
		initGLPBuffer(capabilities, width, height);
	}

	protected void initGLPBuffer(GLCapabilities capabilities, int width,
			int height) {
		...
		glpBuffer = factory.createGLPbuffer(null, capabilities, null, width,
				height, null);
	...
	protected void initGLPBuffer(int width, int height) {
		GLCapabilities caps = org.jzy3d.global.Settings.getInstance()
				.getGLCapabilities();
		caps.setDoubleBuffered(false);
		if (!GLDrawableFactory.getFactory(caps.getGLProfile())
				.canCreateGLPbuffer(null))
			throw new RuntimeException("No pbuffer support");

		glpBuffer = GLDrawableFactory.getFactory(caps.getGLProfile())
				.createGLPbuffer(null, caps, null, width, height, null);
		glpBuffer.addGLEventListener(renderer);
	}
	...
	public String getDebugInfo() {
		...
		sb.append("Chosen GLCapabilities: "
				+ glpBuffer.getChosenGLCapabilities() + "\n");
