...
public class DualDepthPeelingAlgorithm extends AbstractDepthPeelingAlgorithm implements IDepthPeelingAlgorithm{
    ...
    protected ShaderFilePair shaderInit = new ShaderFilePair(DualDepthPeelingAlgorithm.class, "dual_peeling_init_vertex.glsl", "dual_peeling_init_fragment.glsl");
    protected ShaderFilePair shaderPeel = new ShaderFilePair(DualDepthPeelingAlgorithm.class, "dual_peeling_peel_vertex.glsl", "dual_peeling_peel_fragment.glsl");
    protected ShaderFilePair shaderBlend = new ShaderFilePair(DualDepthPeelingAlgorithm.class, "dual_peeling_blend_vertex.glsl", "dual_peeling_blend_fragment.glsl");
    protected ShaderFilePair shaderFinal = new ShaderFilePair(DualDepthPeelingAlgorithm.class, "dual_peeling_final_vertex.glsl", "dual_peeling_final_fragment.glsl");
    ...
    protected void buildShaders(GL2 gl) {
        ...
        //glslInit.loadAndCompileVertexShader(gl, shaderInit.getVertexInputStream(), shaderInit.getVertexURL());
        ...
        glslPeel.loadAndCompileVertexShader(gl, shaderBase.getVertexStream(), shaderBase.getVertexURL());
        glslPeel.loadAndCompileVertexShader(gl, shaderPeel.getVertexStream(), shaderPeel.getVertexURL());
        glslPeel.loadAndCompileFragmentShader(gl, shaderBase.getFragmentStream(), shaderBase.getFragmentURL());
        glslPeel.loadAndCompileFragmentShader(gl, shaderPeel.getFragmentStream(), shaderPeel.getFragmentURL());
        glslPeel.link(gl);

        glslBlend = new GLSLProgram();
        glslBlend.loadAndCompileVertexShader(gl, shaderBlend.getVertexStream(), shaderBlend.getVertexURL());
        glslBlend.loadAndCompileFragmentShader(gl, shaderBlend.getFragmentStream(), shaderBlend.getFragmentURL());
        glslBlend.link(gl);
    ...
    protected void initDualPeelingRenderTargets(GL2 gl, int g_imageWidth, int g_imageHeight) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            gl.glTexImage2D(GL2.GL_TEXTURE_RECTANGLE_ARB, 0, GL2.GL_FLOAT_RG32_NV, g_imageWidth, g_imageHeight, 0, GL2.GL_RGB, GL2.GL_FLOAT, null);
            ...
            gl.glTexParameteri(GL2.GL_TEXTURE_RECTANGLE_ARB, GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_NEAREST);
            gl.glTexParameteri(GL2.GL_TEXTURE_RECTANGLE_ARB, GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_NEAREST);
            gl.glTexImage2D(GL2.GL_TEXTURE_RECTANGLE_ARB, 0, GL2.GL_RGBA, g_imageWidth, g_imageHeight, 0, GL2.GL_RGBA, GL2.GL_FLOAT, null);
        ...
        gl.glTexParameteri(GL2.GL_TEXTURE_RECTANGLE_ARB, GL2.GL_TEXTURE_MIN_FILTER, GL2.GL_NEAREST);
        gl.glTexParameteri(GL2.GL_TEXTURE_RECTANGLE_ARB, GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_NEAREST);
        gl.glTexImage2D(GL2.GL_TEXTURE_RECTANGLE_ARB, 0, GL2.GL_RGB, g_imageWidth, g_imageHeight, 0, GL2.GL_RGB, GL2.GL_FLOAT, null);

        gl.glGenFramebuffers(1, g_dualBackBlenderFboId, 0);
        gl.glBindFramebuffer(GL2.GL_FRAMEBUFFER, g_dualBackBlenderFboId[0]);
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT0, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualBackBlenderTexId[0], 0);

        gl.glBindFramebuffer(GL2.GL_FRAMEBUFFER, g_dualPeelingSingleFboId[0]);

        int j = 0;
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT0, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualDepthTexId[j], 0);
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT1, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualFrontBlenderTexId[j], 0);
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT2, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualBackTempTexId[j], 0);

        j = 1;
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT3, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualDepthTexId[j], 0);
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT4, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualFrontBlenderTexId[j], 0);
        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT5, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualBackTempTexId[j], 0);

        gl.glFramebufferTexture2D(GL2.GL_FRAMEBUFFER, GL2.GL_COLOR_ATTACHMENT6, GL2.GL_TEXTURE_RECTANGLE_ARB, g_dualBackBlenderTexId[0], 0);
    }
