...
public class Util {
	...
	public void debugDepthBufferTo(GL gl, String depthBufferOutput) {
		// Draw depth buffer
		...
		// Context.
		MemoryImageSource depthBuffer = new MemoryImageSource(gl.Context.Viewport.Width, gl.Context.Viewport.Height, color, 0, gl.Context.Viewport.Width);
		if(gl instanceof jgl.wt.awt.GL) {
			jgl.wt.awt.GL awt = (jgl.wt.awt.GL)gl;
			Image jGLDepthBuffer = awt.glJGetComponent().createImage(depthBuffer);
		BufferedImage depthOut = new BufferedImage(jGLDepthBuffer.getWidth(null), jGLDepthBuffer.getHeight(null), BufferedImage.TYPE_INT_ARGB);
