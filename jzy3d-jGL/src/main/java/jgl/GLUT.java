...
//    private GLCanvas JavaCanvas;
  ...
  /*
   * private void error (String s) { // Cancel the "errfunc" in aux of OpenGL
   * System.out.println (s); System.exit (1); }
   * 
   * private void DIFF3 (double p[], double q[], double diff[]) { diff[0] = p[0] -
   * q[0]; diff[1] = p[1] - q[1]; diff[2] = p[2] - q[2]; }
   * 
   * private void crossprod (double v1[], double v2[], double prod[]) { double p[]
   * = new double [3];
   */
  /* in case prod == v1 or v2 */
  /*
   * p[0] = v1[1]*v2[2] - v2[1]*v1[2]; p[1] = v1[2]*v2[0] - v2[2]*v1[0]; p[2] =
   * v1[0]*v2[1] - v2[0]*v1[1]; prod[0] = p[0]; prod[1] = p[1]; prod[2] = p[2]; }
   * 
   * private void normalize (double v[]) { double d;
   * 
   * d = Math.sqrt (v [0] * v [0] + v [1] * v [1] + v [2] * v [2]); if (d == 0.0)
   * { error ("normalize: zero length vector"); v [0] = d = 1.0; } d = 1/d; v[0]
   * *= d; v[1] *= d; v[2] *= d; }
   * 
   * private void m_xformpt (double pin [], double pout [], double nin [], double
   * nout []) { int i; double ptemp [] = new double [3]; double ntemp [] = new
   * double [3]; mat_t m = matstack [mattop];
   * 
   * if (identitymat) { for (i = 0; i < 3; i++) { pout [i] = pin [i]; nout [i] =
   * nin [i]; } return; } for (i = 0; i < 3; i++) { ptemp [i] = pin [0] * m.mat
   * [0][i] + pin [1] * m.mat [1][i] + pin [2] * m.mat [2][i] + m.mat[3][i]; ntemp
   * [i] = nin [0] * m.norm [0][i] + nin [1] * m.norm [1][i] + nin [2] * m.norm
   * [2][i]; } for (i = 0; i < 3; i++) { pout [i] = ptemp [i]; nout [i] = ntemp
   * [i]; } normalize(nout); }
   * 
   * private void m_xformptonly (double pin[], double pout[]) { int i; double
   * ptemp[] = new double [3]; mat_t m = matstack [mattop];
   * 
   * if (identitymat) { for (i = 0; i < 3; i++) { pout[i] = pin[i]; } return; }
   * for (i = 0; i < 3; i++) { ptemp[i] = pin[0]*m.mat[0][i] + pin[1]*m.mat[1][i]
   * + pin[2]*m.mat[2][i] + m.mat[3][i]; } for (i = 0; i < 3; i++) { pout[i] =
   * ptemp[i]; } }
   * 
   * private void recorditem (double n1 [], double n2 [], double n3 [], double
   * center [], double radius, int shadeType, int avnormal) { double p1 [] = new
   * double [3]; double p2 [] = new double [3]; double p3 [] = new double [3];
   * double q0 [] = new double [3]; double q1 [] = new double [3]; double n11 [] =
   * new double [3]; double n22 [] = new double [3]; double n33 [] = new double
   * [3]; int i;
   * 
   * for (i = 0; i < 3; i++) { p1[i] = n1[i]*radius + center[i]; p2[i] =
   * n2[i]*radius + center[i]; p3[i] = n3[i]*radius + center[i]; } if (avnormal ==
   * 0) { diff3(p1, p2, q0); diff3(p2, p3, q1); crossprod(q0, q1, q1);
   * normalize(q1); m_xformpt(p1, p1, q1, n11); m_xformptonly(p2, p2);
   * m_xformptonly(p3, p3);
   * 
   * JavaGL.glBegin (shadeType); JavaGL.glNormal3dv(n11); JavaGL.glVertex3dv(p1);
   * JavaGL.glVertex3dv(p2); JavaGL.glVertex3dv(p3); JavaGL.glEnd(); return; }
   * m_xformpt(p1, p1, n1, n11); m_xformpt(p2, p2, n2, n22); m_xformpt(p3, p3, n3,
   * n33);
   * 
   * JavaGL.glBegin (shadeType); JavaGL.glNormal3dv(n11); JavaGL.glVertex3dv(p1);
   ...
   * double p0 [], double radius, int shadeType, int avnormal) { double w0 [] =
   * new double [3]; double w1 [] = new double [3]; double w2 [] = new double [3];
   * double l; int i, j, k, n;
   * 
   * for (i = 0; i < depth; i++) { for (j = 0; i + j < depth; j++) { k = depth - i
   * - j; for (n = 0; n < 3; n++) { w0[n] = (i*v0[n] + j*v1[n] + k*v2[n])/depth;
   * w1[n] = ((i+1)*v0[n] + j*v1[n] + (k-1)*v2[n])/depth; w2[n] = (i*v0[n] +
   * (j+1)*v1[n] + (k-1)*v2[n])/depth; } l = Math.sqrt(w0[0]*w0[0] + w0[1]*w0[1] +
   * w0[2]*w0[2]); w0[0] /= l; w0[1] /= l; w0[2] /= l; l = Math.sqrt(w1[0]*w1[0] +
   * w1[1]*w1[1] + w1[2]*w1[2]); w1[0] /= l; w1[1] /= l; w1[2] /= l; l =
   * Math.sqrt(w2[0]*w2[0] + w2[1]*w2[1] + w2[2]*w2[2]); w2[0] /= l; w2[1] /= l;
   * w2[2] /= l; recorditem(w1, w0, w2, p0, radius, shadeType, avnormal); } } for
   * (i = 0; i < depth-1; i++) { for (j = 0; i + j < depth-1; j++) { k = depth - i
   * - j; for (n = 0; n < 3; n++) { w0[n] = ((i+1)*v0[n] + (j+1)*v1[n] +
   * (k-2)*v2[n])/depth; w1[n] = ((i+1)*v0[n] + j*v1[n] + (k-1)*v2[n])/depth;
   * w2[n] = (i*v0[n] + (j+1)*v1[n] + (k-1)*v2[n])/depth; } l =
   * Math.sqrt(w0[0]*w0[0] + w0[1]*w0[1] + w0[2]*w0[2]); w0[0] /= l; w0[1] /= l;
   * w0[2] /= l; l = Math.sqrt(w1[0]*w1[0] + w1[1]*w1[1] + w1[2]*w1[2]); w1[0] /=
   * l; w1[1] /= l; w1[2] /= l; l = Math.sqrt(w2[0]*w2[0] + w2[1]*w2[1] +
   * w2[2]*w2[2]); w2[0] /= l; w2[1] /= l; w2[2] /= l; recorditem(w0, w1, w2, p0,
   * radius, shadeType, avnormal); } } }
   */
...
//	    drawtriangle (i, idata, index, shadeType);
  ...
  public void glutAddSubMenu(String label, int submenu) {
    ...
    if ((subMenu.glutGetMenu().getLabel() == null) || (subMenu.glutGetMenu().getLabel().length() == 0)) {
  ...
  /**
   * Process component events, i.e. events sent by the {@link GLCanvas} displaying the GL image generated by {@link GL#glFlush()}.
   * 
   * Basically invoke {@link this#reshapeMethod} and {@link this#displayMethod} to update canvas upon {@link ComponentEvent#COMPONENT_RESIZED}.
   * 
  ...
  public void processKeyEvent(KeyEvent e) {
    ...
    case KeyEvent.KEY_RELEASED:
      ...
      break;
    /*
     * case KeyEvent.KEY_TYPED: invokeKeyMethod (keyMethod, (char)e.getKeyChar());
     * break;
     */
    }
  }
...
//	    } else {
      Object[] arguments = new Object[] { new Integer(button), new Integer(state), new Integer(e.getX()),
          new Integer(e.getY()) };
      try {
...
//	}
   ...
   * 
   * Basically invoke {@link this#motionMethod} upon {@link MouseEvent#MOUSE_MOVED} and {@link MouseEvent#MOUSE_DRAGGED}.
   * 
   * Registering a reshape or display callback is done through
   * 
   * <pre><code>
  ...
  /** 
   * Register the name of the {@link JavaComponent} method that should be called upon component resize events.
   * 
  ...
  /**
   * glut Functions Java is not C language, not all the glut functions can be
