...
public class gl_pointer {
	...
	/**
	 *
	 * Current Status The status of GL about selection, point size, texturing,
	 * smooth or flat shading, depth buffer, and clip-plane clipping.
	 *
	 * Status Select Point Texture Smooth Depth Clipping
	 * --------------------------------------------------- ??00?0 no no no ??00?1 no
	 * no yes 0?000? no no no no 0?001? no no no yes ??01?0 no yes no ??01?1 no yes
	 * yes 0?010? no no yes no 0?011? no no yes yes ??10?0 yes no no ??10?1 yes no
	 * yes 0?100? no yes no no 0?101? no yes no yes ??11?0 yes yes no ??11?1 yes yes
	 * yes 0?110? no yes yes no 0?111? no yes yes yes 00???? no no 01???? no yes
	 * 1----? yes --- --- --- ---
	 *
...
//		    clipping = new gl_cp_tex_clipping (CC);
			...
			if ((status & 0x00000001) != 0) {
				/*
				 * if ((status & 0x00000008) != 0) { clipping = new gl_nf_tex_clipping (CC); }
				 * else { if ((status & 0x00000004) != 0) { clipping = new gl_nf_color_clipping
				 * (CC); } else { clipping = new gl_nf_clipping (CC); } }
				 */
				clipping = null;
				status &= 0xfffffffe;
			}
		...
		if (state) {
			if ((status & 0x00000002) == 0) { // test for depth
				if ((status & 0x00000008) != 0) { // test for texture
					...
					/*
					 * geometry = new gl_tex_z_geo (CC, this); if ((status & 0x00000020) == 0) { //
					 * test for select render = new gl_tex_z (CC); }
					 */
				} else {
			...
			if ((status & 0x00000002) != 0) {
				if ((status & 0x00000008) != 0) {
					...
					/*
					 * geometry = new gl_tex_geo (CC, this); if ((status & 0x00000020) == 0) {
					 * render = new gl_tex (CC); }
					 */
				} else {
					if ((status & 0x00000004) != 0) {
	...
	public void gl_texture(boolean state) {
		if (state) {
			if ((status & 0x00000008) == 0) {
				...
				/*
				 * if ((status & 0x00000002) != 0) { if ((status & 0x00000020) == 0) { render =
				 * new gl_tex_z (CC); } geometry = new gl_tex_z_geo (CC, this); } else { if
				 * ((status & 0x00000020) == 0) { render = new gl_tex (CC); } geometry = new
				 * gl_tex_geo (CC, this); } if ((status & 0x00000001) != 0) { clipping = new
				 * gl_cp_tex_clipping (CC); }
				 */
				/*
				 * else { clipping = new gl_nf_tex_clipping (CC); } vp_clipping = new
				 * gl_vp_tex_clipping (CC);
				 */
				status |= 0x00000008;
			}
		} else {
		...
		if (state) {
			...
			if ((status & 0x00000020) != 0) {
				if ((status & 0x00000008) != 0) {
					...
					/*
					 * if ((status & 0x00000002) != 0) { render = new gl_tex_z (CC); } else { render
					 * = new gl_tex (CC); }
					 */
				} else {
					if ((status & 0x00000002) != 0) {
